...
Kafka 只是分为一个或多个分区的主题的集合。Kafka 分区是消息的`线性有序序列`，其中每个消息由它们的`索引(称为偏移)`来标识。Kafka 集群中的所有数据都是不相连的分区联合。 传入消息写在分区的末尾，消息由消费者顺序读取。 通过将消息复制到不同的代理提供持久性。
...
- 一旦新消费者到达，Kafka 将其操作切换到共享模式，并在两个消费者之间共享数据。 此共享将继续，直**到用户数达到为该特定主题配置的分区数**。
